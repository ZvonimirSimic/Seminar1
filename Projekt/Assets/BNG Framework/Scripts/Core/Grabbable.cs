...
namespace BNG {
    ...
    public class Grabbable : MonoBehaviour {
        protected Vector3 initPos, initSca;
        protected Quaternion initRot;
        protected bool twoHandsEnd = true;
        /// <summary>
        ...
        public void scaleHeldObject()
        {
            if (BeingHeld && BeingHeldWithTwoHands)
            {
                twoHandsEnd = false;
                Vector3 first = GetGrabberVector3(GetPrimaryGrabber(), false);
                Vector3 second = GetGrabberVector3(GetPrimaryGrabber(), true);
                second = second - first;
                /*transform.localScale.x = (initSca.x + second.x)/this.transform.parent.transform.lossyScale.x;
                transform.localScale.y = (initSca.y + second.y) / this.transform.parent.transform.lossyScale.y;
                transform.localScale.z = (initSca.z + second.z) / this.transform.parent.transform.lossyScale.z;*/
                transform.localScale = new Vector3((initSca.x + second.x) / this.transform.parent.transform.lossyScale.x, (initSca.y + second.y) / this.transform.parent.transform.lossyScale.y, (initSca.z + second.z) / this.transform.parent.transform.lossyScale.z);
                if (ControllerBinding.LeftTriggerDown.GetDown() || ControllerBinding.RightTriggerDown.GetDown())
                {
        ...
        public virtual void FixedUpdate() {
            if (twoHandsEnd)
            {
                ...
                initSca = transform.localScale;         // SREDI
            ...
            if (BeingHeld) {
                if(ControllerBinding.LeftTriggerDown.GetDown() || ControllerBinding.RightTriggerDown.GetDown())
                {
                    ...
                    string str = inputfield.gameObject.GetComponent<VRKeys.Keyboard>().text;
                    if (GetComponentInChildren<TextMeshPro>() != null)
                    {
                        GetComponentInChildren<TextMeshPro>().text = str;
