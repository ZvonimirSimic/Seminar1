            ...
            if (onButtonDown != null)
            {
                if (pressed == false)
                {
                    ...
                    for (int i = 1; i < childrenList.Count; i++)
                    {
                        ...
                        childrenList[i].GetComponent<MeshCollider>().sharedMesh = combiner(childrenList[i]);
                        gamObjs[i] = Instantiate(childrenList[i], newPos + childrenList[i].transform.localPosition / 50, childrenList[i].transform.rotation);
                        gamObjs[i].transform.localScale = new Vector3(gamObjs[i].transform.localScale.x, gamObjs[i].transform.localScale.y, gamObjs[i].transform.localScale.z);
